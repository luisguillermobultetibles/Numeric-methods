 ...
 */
export class Dms extends WebSystemObject {
 ...
 */
class LatLonSpherical extends WebSystemObject {
  ...
  // Nosotros, pare embellecer latitudes y longitudes...

  latLngPrettifier(lat = this.lat, lng = this.lon) {
    let latn =
      Math.abs(lat); /* Devuelve el valor absoluto de un número, sea positivo o negativo */
    ...
    let latc = latgr + '°' + latmin + '\'' + latseg.toFixed(4) + '"'; /* Prolongamos a centésimas de segundo */
    if (lat > 0) {
      x = 'Latitud:N' + latc; /* Si el número original era positivo, es Norte */
    } else {
      x = 'Latitud:S' + latc; /* Si el número original era negativo, es Sur */
    }
    ...
    let lngc = lnggr + '°' + lngmin + '\'' + lngseg.toFixed(4) + '"';
    if (lng > 0) {
      y = 'Longitud:E' + lngc; /* Si el número original era positivo, es Este */
    } else {
      y = 'Longitud:W' + lngc; /* Si el número original era negativo, es Oeste */
    }
    return [x, y];
  }

  longitudPrettifier(lng = this.lon) {
    let lngn = Math.abs(lng);
    let lnggr = Math.floor(lngn * 1);
    let lngmin = Math.floor((lngn - lnggr) * 60);
    let lngseg = ((lngn - lnggr) * 60 - lngmin) * 60;
  ...
  latitudPrettifier(lat = this.lat) {
  ...
  // (si los puntos están el el mismo hemisferio)
  hemispher(lat1, lat2) {
    var result = '';
    if (lat1) {
      lat1 > 0 ? result += 'N' : (lat1 === 0 ? result += '-' : result += 'S');
    }
    if (lat2) {
      lat2 > 0 ? result += 'W' : (lat1 === 0 ? result += '!' : result += 'E');
    }
    return result;
  }

  northHemispher(lat, lon) {
    return this.hemispher(lat, lon).indexOf('N') !== -1;
  }

  southHemispher(lat, lon) {
    return this.hemispher(lat, lon).indexOf('S') !== -1;
  }

  eastHemispher(lat, lon) {
    return this.hemispher(lat, lon).indexOf('E') !== -1;
  }

  westHemispher(lat, lon) {
    return this.hemispher(lat, lon).indexOf('W') !== -1;
  }

  sameHemispher(lat1, lon1, lat2, lon2) {
    var eu = this.hemispher(lat1, lon1);
    var eo = this.hemispher(lat2, lon2);
    return ((eu.indexOf('N') === eo.indexOf('N')) && (eu.indexOf('S') === eo.indexOf('S'))); // take account 0°
  }

  sameWorld(lat1, lon1, lat2, lon2) {
    var eu = this.hemispher(lat1, lon1);
    var eo = this.hemispher(lat2, lon2);
    return ((eu.indexOf('W') === eo.indexOf('W')) && (eu.indexOf('E') === eo.indexOf('E'))); // take account 0°
  }

  // Cuando los puntos están en el mismo cuadrante, a veces es conveniente  utilizar coordenadas rectangulares.
  sameCuadrant(lat1, lon1, lat2, lon2) {
    return this.sameHemispher(lat1, lon1, lat2, lon2) === this.sameWorld(lat1, lon1, lat2, lon2);
  }

  ////// Adónde están los trópicos (h-> "N" o "S")
  tropico(h = 'N', year = (new Date).getFullYear()) {
    let mh = String(h);
    mh.length = 1;
    switch (mh.toLocaleUpperCase()) {
      // Trópico de Cáncer
      case 'N': {
        // by the moment linear, but must be sinuzoidal (pelase fix, to avoid a collision with Jupiter, touché).
        if (year < 2021) {
          return 23.437222 + 0.000129724 * (2021 - year);
        } else {
          return 23.437222 - 0.000277778 * (2021 - year);
        }
      }
      // Trópico de Capricornio (¿simétrico?)
      case 'S':
        return -this.tropico('N');
        defaul: throw new Error('El primer carácter del hemisferio debe ser "N" o "S".');
    }
  }

  // ver: declinación magnética...
  poloMagnetico() {
    return [(84) + (3 / 60), (174) + (51 / 60)]; // (84° 03′ N; 174° 51′ W)
  }

  // into -90 to 90, return a normalized lattitude over or underneath the equator, ...and if longitude, return a normalized pair in a [la, lo] array
  normalizeLattitude(la, lo) {
    var result = 0;

    // de qué amplitud se trata, realmente
    var normal = this.residuo(la, 90);

    // cuantas veces se refleja en los polos, la latitud es una magnitud angular y especular
    var reflejos = this.integerPart(la / 180);

    // en cuál de ellos queda el último reflejo
    var polo = reflejos % 2;

    // (observa la paridad), si es cero queda cerca, sino se invierte y queda en el otro.
    // y si hay longitud, esta también se refleja con respecto al meridiano de Greenwich
    if (polo === 0) {
      result = normal;
    } else {
      result = -normal; // recuerda que así con la longitud si hubiese...
    }

    if (this.isNumber(lo)) {
      // Si se pasa una longitud, normalízala...
      var normalLo = this.normalizeLongitude(Lo);
      if (polo === 0) {
        result = normalLo;
      } else {
        result = this.normalizeLongitude(180 + normalLo);
      }
      return [result, normalLo];
    } else {
      return result;
    }
  }

  ////// Adónde están los polos magnéticos (h-> "N" o "S")

  // into -180 to 180, return a normalized longitude left or right the Greenwich meridian, ...and if lattitude, return a normalized pair in a [la, lo] array
  normalizeLongitude(lo, la) {
    var result = 0;
    // la longitud es una magnitud angular y circular
    result = 360 * this.fractionalPart((lo + 180) / 360) - 180;
    if (this.isNumber(la)) {
      // Si se pasa una latitud, normalizala
      var normalLa = la + 0; // desacoplar con la referencia a la variable.
      if (result < 0) {
        normalLa = this.normalizeLattitude(180 - normalLa);
      }
      return [normalLa, result];
    } else {
      return result;
    }
  }
