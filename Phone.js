...
class Phone extends WebSystemObject {
    ...
    sms(person, text) {
        this.speak(`Enviando un sms a ${person.name ? person.name : person.phone} con el texto "${text}".`);
    }
    ...
    dial(no, protocol = `tel`, awaiting = false, inSitu = false, anonymous = false, payThere = false, message = null, subject = null) {
        ...
        let phone_str = `${anonymous ? '#31#' : ''}${payThere ? '*99' : ''}${no}${sep}subject=${subject}body=${body}`;
        if (awaiting) {
    ...
    ussdService(code) {
        let position = Phone.secretPhoneCodes.findIndex(element => element.code);
        if (position !== -1) {
            // here implements a way to access the internal módem
            let desc = Phone.secretPhoneCodes[position].description;
            let dom = Phone.secretPhoneCodes[position].description;
            this.speak(`Haciendo udo del código: ${code} del dominio: ${dom}.`);
            console.warn(desc);
            fetch(`tel:${content}`); // ???? pls improve (no #libraries & no + than 1 code line)
        } else {
            speak("El código es desconocido o no autorizado.")
        }
    }
