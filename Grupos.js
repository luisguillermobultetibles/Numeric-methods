...
export class Grupo extends WebSystemObject {
  ...
  static Arreglo = class extends Grupo {
    /**
     * initialize MyArray
     *
    ...
    random() {
      return this._array[super.random(0, this._array.length - 1)];
    }
    ...
    // clonar
    clone() {
      const resultado = new Arreglo(this._array.count);
    ...
    */
    ...
    sumSet(capacity, eps = this.epsilon) {
      ...
      for (let i = 0; i < this._array.length - 1; i++) {
        ...
        subSolution = this.sumSet(capacity - this._array[i], subSet);
    ...
    topologicalSort() {
      ...
      let depVis = (task1, task2) => {
        ...
        if (this._array.some((element) => element.name === task1.name && element.dependencies.indexOf(task2.name) !== -1)) {
          return dependant;
        } else {
          return task1.dependencies.some((element) => {
            const it = this._array.find((el) => element === el.name);
            return depVis(task1, it) === dependant && depVis(it, task2) === dependant;
          }) ? dependant : notDependant;
        }
      };

      return this._array.sort((a, b) => a.name < b.name ? dependant : notDependant).sort((task1, task2) => -depVis(task1, task2)).reverse();
    }
  };
  static Conjunto = class extends Grupo {
    constructor() {
      super(...arguments);
    }
  };
  static Distribucion = class extends Grupo {
    constructor() {
      super(...arguments);
    }
  };
  static Poblacion = class extends Grupo {
    constructor() {
      super(...arguments);
    }
  };
...
// Alias
export const Arreglo = Grupo.Arreglo;
export const Conjunto = Grupo.Conjunto;
export const Distribucion = Grupo.Distribucion;
export const Poblacion = Grupo.Poblacion;
