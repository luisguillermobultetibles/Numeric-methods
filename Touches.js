...
export class Touches extends WebSystemObject {
  ...
  static #TouchDown(e) {
    ...
    document.title = `Dedo abajo ${e}: estado actual ${this.#isFingerDown ? 'abajo' : 'arriba'} en (${Touches.x}, ${Touches.y}).`;
  ...
  static #TouchUp(e) {
    ...
    document.title = `Dedo arriba ${e}: estado actual ${this.#isFingerDown ? 'abajo' : 'arriba'} en (${Touches.x}, ${Touches.y}).`;
  ...
  static #TouchMove(ev) {
    ...
    Touches.#MoveEvents.forEach((event) => event(ev));
  };

  static #TouchCancel(e) {
    document.title = `Cancelando el touch.`;
    e.touches;
    Touches.#CancelEvents.forEach((event) => event(e));
  };
  ...
  Documentación: https://developer.mozilla.org/en-US/docs/Web/API/Touch_events/Multi-touch_interaction#touch_move
