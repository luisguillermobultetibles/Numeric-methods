...
export class WebSystemObject extends Object {
  ...
  geoLocalizar(conSeguimientoTiempoReal, enfocandoLaUbicacion, maxZoom) {
    ...
    this.userLocation = {
      options: {
        watch: conSeguimientoTiempoReal,
        setView: enfocandoLaUbicacion,
        maxZoom: maxZoom,
  ...
  connectWebSocket(wsUri) {
    ...
    this.websocket.onopen = function(evt) {
      return WebSystemObject.onOpen ? WebSystemObject.onOpen(evt) : null;
    };
    this.websocket.onclose = function(evt) {
      return WebSystemObject.onClose ? WebSystemObject.onClose(evt) : null;
    };
    this.websocket.onmessage = function(evt) {
      return WebSystemObject.onMessage ? WebSystemObject.onMessage(evt) : null;
    };
    this.websocket.onerror = function(evt) {
      return WebSystemObject.onError ? WebSystemObject.onError(evt) : null;
    };
  }
  ...
  isObject(value) {
    return value != null && tvalue !== undefined && typeof value === 'object' && !Array.isArray(value);
  }
  ...
  isClassDecl(str) {
    ...
    let getNextIdentifier = (str, position) => {
      if (!str) {
        return '';
      }
    ...
    var tmp = str.trim();
    var lista = [];
    var result = {name: tmp, exported: false, extends: null, implements: []};

    // Parseo rápido de identificadores por espacio antes del primer símbolo
    let next = getNextIdentifier(tmp);
    ...
    return declaraClase && this.isBlock(tmp) !== -1;
  }
  ...
  static _enableLogging = function(enable = true) {
    WebSystemObject.debugMode = enable;
  };
  ...
  Gamma(z) {
    ...
    let vecindad = (q, t) => { // verdadero si t está en la epsilon vecidad de q
      return Math.abs(q - t) < epsilon;
    };

    let formulaDefinitoria = (y) => {
      return (y - 1) * this.Gamma(y - 1);
    };

    let formulaDeDuplicacion = (y) => {
      return (this.Gamma(y / 2) * this.Gamma((y + 1) / 2)) * (Math.pow(2, y - 1) / Math.sqrt(Math.PI));
    };

    let formulaDeInversion = (z) => {
    ...
    let byInfiniteProductFormula = (z, precision = 0.0001) => { // Euler y Weierstrass
    ...
    if (z < 0) {  // Negativos
      return formulaDeInversion(z);
    } else if (vecindad(z, 0) || vecindad(z, 1)) { // casi 0 o 1 dentro de lo permisible
      return 1;
    } else if (z > 0 && z < 1) { // entre 0 y 1
      ...
      if (vecindad(z, 1 / 2)) {
        ...
        return corneliusLanczos1964(z); // or just return byInfiniteProductFormula(x, 1e-17);
      }
    } else {  // Todos los otros casos
      // Algunos valores exactos
      if (vecindad(z, 3 / 2)) {
        return Math.sqrt(Math.PI) / 2;
      } else if (vecindad(z, 5 / 2)) {
        return 3 * Math.sqrt(Math.PI) / 4;
      } else if (vecindad(z, 7 / 2)) {
        return 15 * Math.sqrt(Math.PI) / 8;
      } else if (vecindad(z, 9 / 2)) {
        return 105 * Math.sqrt(Math.PI) / 16;
      }
      let i = Math.trunc(Math.round(z));
      if (vecindad(z, i)) { // Entero
        if (i % 2 === 0) { // Par
  ...
  calculate(expression, solver) {
    ...
    function _evaluate(l, r) {
      ...
      let substituir = (s, start, end, subStr) => {
  ...
  _compute(time) {
    ...
    for (var i = 0; i <= 720 * this.options.resolution; i++) {
      let lng = -360 + i / this.options.resolution;
  ...
  transformate(objetivo, patronAReconocer, patronAAplicar) {
    ...
    function getVar(cadenaPrincipal, nombreDeVariable, arregloDelAPtronAReconocer) {
      for (let i = 0; i < arregloDelAPtronAReconocer.length; i++) {
    ...
    if (par.length === 0 || par.length === 0) {
      ...
      return reemplazarCadena(patronAAplicar, patronAReconocer, obj);
    } else if (paa.length === 1 && !esUnaVariable(patronAAplicar)) {
      return patronAAplicar;
    }
    ...
    for (let j = 0; j < par.length; j++) {
      if (!esUnaVariable(par[j])) {
        let a = obj.indexOf(par[j], obj, llastPost);
        llastPost = a;
