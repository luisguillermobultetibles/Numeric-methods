    ...
    function genericParse(v, options = { functionCallback: null, variableCallbak: null }) {
        let result = [];
        let token = '';
        let counter;
        let pushToken = (type = tokenTypeUnkKnown) => {
            let tmp = token;
            token = '';
            let tok = { source: tmp, content: tmp, type: type }; // as obj
        ...
        let classClosers = [')', '}', ']', '"', "'", '`', "/"];
        let classSymbols = [";", ".", "@", "#", "$", "%", "^", "&", "*", "~", ":", "{", "}", "=", "[", "]", ",", ">", "<"];
        let classOperators = ["instanceof", "typeof", ">>>=", ">>>", ">>=", "<>=", "===", "!==", "<=", ">=", "&&", "||", "++", "--", "+=", "-=", "*=", "/=", "%=", "^=", "&=", "!=", "|=", "&", "|", "^", "<", ">", "!", "~", "+", "-", "*", "/", "%", ";", ",", "=>"];
        let isOperatorAt = (so, p = 0, _class = classOperators) => {
        ...
        let type = (so, p = 0) => {
            ...
            let char = classOpeners.indexOf(so[p]);
            switch (char) {
                case 0:
        ...
        let sintaxCheck = (cadena, posicion) => {
            ...
            if (cadena.substr(posicion, 2) === "//") { // Line comment, until CR or EOF
                ...
                let endComment = cadena.indexOf("*/", posicion);
                if (endComment !== -1) {
