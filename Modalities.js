...
const proposicionAtributiva = 1; // Se afirma o se niega que un objeto posee ciertas propiedades, estados o tipos de actividad, siguen el esquema: "S es P" o "S no es P". (la rosa es roja).
const proposicionRelacional = 2; // Se refieren a las relaciones entre los objetos, siguen el esquema: "A R P" o "S no R P". ef (a > b, x=2, "el protón es mas pesado que el neutrón").
const proposicionExistencial = 3; // Afirman o niegan la existencia de otros objetos (sucesos o fenómenos).

// TIPOS DE ARGUMENTOS (Se usan poco, y van bien grandes porque son esenciales).
const argumentoLEY_AXIOMA_O_POSTULADO = 1; // Por ejemplo, las leyes de la robótica, las carta de derechos de las naciones unidas (exceptuando los vetos), luego la constitución de la república, las leyes que se derivan, las necesidades básicas personales, los estados de necesidad, sin intermedios y en ese orden., luego the user, then homo sapien, humano, nacido, cubano, vivo, por último, las tortugas, nosotros, los salvavidas y desarrolladores.
const argumentoHECHO_SINGULAR_CERTIFICADO_POR_LA_SOCIEDAD_O_LA_COMUNIDAD_CIENTIFICA = 2;
const argumentoDEFINICION = 3;
const argumentoTEOREMA = 4;
const argumentoHECHO_SINGULAR_CERTIFICADO_POR_EL_USUARIO = 5; // Que lo vió con estos ojos que tengo aquí...
const argumentoHECHO_SINGULAR_CERTIFICADO_POR_OTROS_USUARIOS = 6; // Que fulano o mengano y sutano lo dijeron...
const argumentoHECHO_SINGULAR_CERTIFICADO_POR_OTROS_SISTEMAS = 7; // Que salió por tal periódico... (en todos caso siempre es bueno agregar una referencia al origen de la inf.)
...
const operacionIdentidad = 1;
const operacionNegacion = 2;
const operacionConjuncion = 3;
const operacionDisyuncion = 4;
...
// TIPOS DE ORACIONES
const oracionEcunciativa = 1; // Se le puede asignar un valor de verdad
const oracionDeclarativa = 2; // Idem
const oracionImperativa = 3; // Manda (No tiene valor de verdad)
const oracionExhortativa = 3; // Idem
const oracionExclamativa = 4; // Tampoco tienen valor de verdad, tienen finalidad expresiva
const oracionInterrogativa = 5; // Tampoco... sirven para solicitar información.
const oracionDubitativa = 6; // Expresan una suposición o probabilidad.
const oracionOptativa = 7; // Expresan un deseo, se pueden pronunciar de forma exclamativa.
const oracionDesiderativa = 7; // Idem
